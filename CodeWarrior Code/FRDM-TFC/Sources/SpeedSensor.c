...
#define WHEEL_DIAMETER 0.049f 				// Wheel diameter considering tyre compression because of car weight
#define RATE_SCALER 10*2*187500.0f/7.5f		// 10*2*(timer_frequency)/(gear_ratio)																																								
#define MIN_SPEED_DETECTABLE 3.0f
#define MAX_QUARTER_PERIOD  RATE_SCALER / (4.0f*MIN_SPEED_DETECTABLE)
...
float getSpeed(uint8_t channel)
{
	...
	speeds[channel].speed = WHEEL_DIAMETER * (RATE_SCALER / (float) speeds[channel].period);	// Speed in (car speed unit), Real_Speed = speed*PI/20 (m/s)
